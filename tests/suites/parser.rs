...
fn negative_literal_in_body() {
    // `-5` (a `-` glued to digits) is a single negative-literal token, so the body
    // is `num(-5)`, not unary minus applied to `5`.
...
fn unary_minus_on_non_literal() {
    // A `-` before a non-digit is still the unary-minus operator (the negative-
    // literal rule only claims a `-` immediately followed by digits).
