...
/// for the type checker only (see `compile_program`): the checker resolves a
/// call to `map`/`value_or`/`print`/… against them exactly as it would a user
/// function, with no notion that they're builtin. Each body is a trivial value
/// of the declared return type so it type-checks like any function — it is never
/// compiled (monomorphization and codegen lower the real implementations).
///
...
const BUILTIN_SIGNATURES: &str = r#"
...
/// source is a fixed constant, so a parse failure is a compiler bug. Each is
/// marked `pub` to match the hand-built originals (visibility is irrelevant to
/// the checker, the only consumer).
